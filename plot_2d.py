...
for f in files[:10]:
    ...
    if len(img[:,3]) == 40401:
        ...
        img[img < 1e-27] = 0 # outsize numerical accuray... creates noise in plots
        ...
        if unit == '96':
            ...
            plt.xlabel(f'x [$\mu m$]')
            plt.ylabel(f'y [$\mu m$]')
            plt.title('Displacements per atom')
            plt.tight_layout()
        ...
        if unit == '97':
            ...
            plt.imshow(img,
                       ...
                       extent=[0, int(bt), -50, 50],
                       aspect=int(bt)/100,
                       norm=colors.LogNorm(vmin=1e-10, vmax=10)
                       )
            plt.colorbar()
            plt.xlabel(f'x [$\mu m$]')
            plt.ylabel(f'y [$\mu m$]')
            ...
            plt.tight_layout()
            plt.savefig('pics_energy_deposit' + os.path.sep + fn + '.png')
